...
func TestParseBodyTravel(t *testing.T) {
	...
	distancia, starttime, err := parseBodyTravel(string(file))
	if err != nil {
		log.Fatal(err)
	}
	if distancia != 3.82 {
...
func TestParseBodyTravel2(t *testing.T) {
	...
	if distancia != 0.00 {
		t.Fatalf(`Parse distancia failed. distancia mustbe "0.00" not "%v"`, distancia)
	}
	if start, err := time.ParseDuration("11h24m0s"); err == nil {
		if starttime != start {
			t.Fatalf(`Parse start time failed. starttime mustbe "11h24m0s" not "%v"`, starttime)
		}
	}
}
...
func TestParseBodyPlan(t *testing.T) {
	...
	plan, err := parseBodyPlan(string(file))
	if err != nil {
		log.Fatal(err)
	}
	...
	if plan, err := parseBodyPlan(""); err == nil && plan.Inicio.IsZero() {
...
func TestParseInputDate(t *testing.T) {
	...
	formattedDate := parseAndFormatDate(afterDate, "2006/01/02")
	if formattedDate != afterDate {
		t.Fatalf(`Error en la conversión de fecha de "%v" > "%v"`, afterDate, formattedDate)
	}
	if formattedDate != "2024/12/01" {
